...
This guide explains how to install and configure **ProxPilot 2.3.0** for one or more Proxmox VE environments.

ProxPilot 2.3.0 supports multiple independent Proxmox infrastructures. An infrastructure can be either:
...
The SSH section displays the **ProxPilot SSH public key** together with a copy-ready setup command.

Only the public key is installed on Proxmox. Never copy or expose the private key `ssh/id_ed25519`.

For the default SSH user `root`, copy the complete setup command displayed by ProxPilot and execute it in a shell on each target Proxmox node. The command creates `/root/.ssh` when necessary, applies the required permissions, appends the managed ProxPilot public key to `/root/.ssh/authorized_keys`, and protects the authorization file.

Conceptually, the generated command performs the following steps:
...
Use the actual command generated by the ProxPilot interface rather than the placeholder key shown in this documentation. Appending the key preserves existing authorized keys.
...
Use the copy-ready SSH setup command shown by ProxPilot and execute it on every node ProxPilot should manage. For the default `root` user, the command installs the public key in:

```text
/root/.ssh/authorized_keys
```

The command shown in the interface contains the actual managed ProxPilot public key and can therefore be pasted directly into the target node's shell.
...
Verify migration first with a non-production guest.

For migration inside the same Proxmox cluster, confirm that the guest can be migrated to another eligible cluster node and that the task completes successfully.

ProxPilot 2.3.0 also supports migration of a **stopped QEMU virtual machine between independent configured infrastructures**. Because the source and destination do not share one Proxmox cluster context, ProxPilot performs this as a managed cross-infrastructure operation rather than as a normal Proxmox live migration.

Before starting such a migration, verify:

- the source QEMU VM is stopped
- both source and destination infrastructures are enabled and reachable
- API access works for both infrastructures
- SSH access works to the participating Proxmox nodes
- the selected destination node is reachable
- suitable destination storage is available
- sufficient free storage space is available
- required network and bridge mappings are valid on the destination
- the destination does not already contain a conflicting VMID
- the source and destination environments have the permissions required for the operation

Cross-infrastructure migration uses preflight checks before the destructive or data-moving phase begins. Depending on the source and destination storage layout, temporary staging storage can be required while the VM data is transferred. ProxPilot tracks the migration as a managed task and performs cleanup of temporary migration data after the operation where applicable.

Do not use a production VM as the first cross-infrastructure migration test. Verify the complete workflow with a disposable or otherwise recoverable stopped QEMU VM first.
...
ProxPilot 2.3.0 provides a selectable timezone setting so administrators do not need to enter timezone identifiers manually. The selected timezone is used by application features that require the configured regional timezone.

The `TZ` environment variable remains the container-level default timezone setting.

## Notifications

ProxPilot 2.3.0 supports notification delivery through:
...
## Cross-Infrastructure Migration Fails

Cross-infrastructure migration is different from normal migration between nodes of the same Proxmox cluster.

If migration of a stopped QEMU VM between independent infrastructures fails, verify the preflight requirements first:

- the VM is stopped
- both infrastructures are reachable
- source and destination API credentials are valid
- SSH access works to the participating nodes
- the destination node is reachable from ProxPilot
- destination storage exists and has sufficient free capacity
- destination networking required by the VM is available
- no conflicting destination VMID exists
- temporary staging storage, when required by the selected storage layout, has sufficient capacity

Also inspect the ProxPilot task details and backend logs:

```bash
docker compose logs --tail=200 backend
```

A failed preflight should be corrected before retrying the migration. If a transfer fails after staging has started, inspect the managed task result and verify that temporary migration data has been cleaned up before starting another attempt.
...
- same-cluster migration works where permitted
- cross-infrastructure migration of a stopped test QEMU VM has been verified when this feature will be used
